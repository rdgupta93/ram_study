...
//        Check if a String Contains All Unique Characters - Given a string, check if all characters are unique.
...
//        Find the First Non-Repeating Character in a String - Given a string, find the first non-repeating character using a HashSet.
...
//
//        Hard Level
//        Count Distinct Subsequences - Given a string, count the number of distinct subsequences using a HashSet.
...
//        Word Break Problem - Given a dictionary and a string, check if the string can be segmented into dictionary words.
//        Find the Longest Palindrome that Can Be Built Using Letters of a Given String - Given a string, find the longest palindrome that can be built using its letters.
 ...
 */
    ...
    /*
    // You are given an array of integers and a target sum. Your task is to determine whether there exists a pair of numbers in the array whose sum is equal to the target.
     // arr = [3, 5, 1, 7, 9]
        //target = 10      true;  arr[i]+arr[j] = target

                  int [] arr = {3, 5, 1, 7, 9};
        // Target(arr-target)  7   5  9  3   1
        int target = 10;
    ...
    /*
     //Given an array of integers (both positive and negative), you need to find the length of the longest contiguous subarray whose sum is zero.
      // The sum of elements in this subarray should be zero.
      //Among all possible subarrays with sum zero, you need to return the maximum length.
      // Input: arr = [1, 2, -3, 3, 1, -4, 2, 2]  ans = 3   {3,1,-4}

        int [] arr = {1, 2, -3, 3, 1, -4, 2, 2};
      //  int [] arr ={1,2,-1,1,2,3,-6};
        //     sum    1  3   0  3  4   0   2  2
        ...
        for(int i=0;i< arr.length;i++){
            ...
            if(hm.containsKey(sum)){
                int currLength = i-hm.get(sum);
                if(currLength>length){
                    length = currLength;
 //                   int ar[] = new int[i-hm.get(sum)];
                    al.clear();
                    for(int j=0;j<i-hm.get(sum);j++){
//                        ar[j] =arr[hm.get(sum)+1+j];
                        int val = arr[hm.get(sum)+1+j];
                        al.add(val);
                    }
                }
            }else {
